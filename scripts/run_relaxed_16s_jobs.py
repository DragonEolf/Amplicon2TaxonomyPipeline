...
def run_sample(sample: dict[str, object]) -> None:
    ...
    marker = "16s_v3v4"
    job_id = safe_filename(f"{marker}_paired_{label}_relaxed_{STAMP}")
    ...
    write_status(job_dir, state="running", message="Running DADA2 in relaxed mode", job_id=job_id)
    try:
        ...
        manifest_path = write_manifest(
            ...
            dada2_params=dada2_params_for_mode(marker_config.dada2, "relaxed"),
        )
        ...
        json.dumps(
            {
                "job_id": job_id,
                "state": "completed",
                "message": "Job completed in relaxed mode with min_boot=0 taxonomy",
            },
            indent=2,
        )
    )
